...
% -- e-mail: studer@cornell.edu, sven.jacobsson@ericsson.com, and
% -- oc66@cornell.edu, and Pikachu (version 0.2; August 14, 2017)
% -------------------------------------------------------------------------
...
function POKEMON_MIMO_sim(varargin)
  ...
  if isempty(varargin)
    ...
    par.mod = 'QPSK';    % modulation type: 'BPSK','QPSK','16QAM','64QAM','8PSK'
    ...
    par.beamformer = ... % beamforming scheme(s) to be evaluated
      {'ZF','ZFQ','SQUID','POKEMON'}; 
    par.tmax = 20;       % number of POKEMON iterations
    par.gamma = 1;       % POKEMON's gamma regularization parameter
    par.delta = 0.2;     % POKEMON's delta regularization parameter
                         % (0 < delta < gamma)
  ...
  % vector error rate
  res.VER = zeros(length(par.beamformer),length(par.NTPdB_list));
  % symbol error rate
  res.SER = zeros(length(par.beamformer),length(par.NTPdB_list));
  % bit error rate
  res.BER = zeros(length(par.beamformer),length(par.NTPdB_list));
  % error-vector magnitude
  res.EVM = zeros(length(par.beamformer),length(par.NTPdB_list));
  % SINDR
  res.SINDR = zeros(length(par.beamformer),length(par.NTPdB_list));
  % transmit power
  res.TxPower = zeros(length(par.beamformer),length(par.NTPdB_list));
  % receive power
  res.RxPower = zeros(length(par.beamformer),length(par.NTPdB_list));
  % simulation beamforming time
  res.TIME = zeros(length(par.beamformer),length(par.NTPdB_list));
      ...
      for k=1:length(par.NTPdB_list)
        ...
        switch (par.beamformer{d})      
          ...
          case 'ZFQ',     % ZF beamforming (quantized)
            ...
            x = par.quantizer(x); 
            beta = beta/par.F;
          case 'POKEMON', % PrOjected downlinK bEaMfOrmiNg (POKEMON, 1-bit)
            [x, beta] = POKEMON(par,s,H);
          % noise-dependent
          case 'SQUID',   % Squared inifinity-norm relaxation with 
  ...
  if par.plot      
    ...
    legend(par.beamformer,'FontSize',12,'location','southwest')
    set(gca,'FontSize',12);        
    
    % - computation times
    figure
    for d=1:length(par.beamformer)
      semilogy(par.NTPdB_list,res.TIME(d,:),marker_style{d},'LineWidth',2)
      if d==1        
        hold on
      end        
    end
    ...
    box on
    xlabel('normalized transmit power [dB]','FontSize',12)
    ylabel('average computation time [s]','FontSize',12)
    if length(par.NTPdB_list) > 1
      axis([min(par.NTPdB_list) max(par.NTPdB_list) 1e-4 1e2])
    end
    legend(par.beamformer,'FontSize',12)
    set(gca,'FontSize',12)        
...
%% PrOjected downlinK bEaMfOrmiNg (POKEMON) (1-bit beamforming algorithm)

function [x, beta] = POKEMON(par,s,H)

  % -- real-valued decomposition
  Hbar = [real(H), -imag(H); imag(H), real(H)];
  sbar = [real(s); imag(s)];

  % -- initial guess
  xbar = Hbar'*sbar;

  % -- precompute the matrix required to obtain the new q of each iteration
  Ginv = inv( eye(par.B*2) ... 
              + 1/par.gamma*Hbar' ...
              * (eye(par.U*2)-sbar*(sbar'/norm(sbar,2)^2))*Hbar );  
  % -- precompute the factor to use in the push operation
  factor = par.gamma/(par.gamma-par.delta);
  
  % -- main loop of POKEMON          
  for i=2:par.tmax
    qbar = Ginv * xbar;
    xbar = sign(qbar).*min(factor*abs(qbar),sqrt(1/(2*par.B)));
  end
  
  % -- final 1-bit quantization and re-express xbar as a complex vector x
  xbar = sign(xbar)*sqrt(1/(2*par.B));
  x = xbar(1:par.B) + 1i*xbar(par.B+1:2*par.B);
  
  % -- compute the beamforming factor
  beta = norm(sbar,2)^2/(sbar'*Hbar*xbar);
